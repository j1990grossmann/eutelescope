  ...
  namespace Utility {
    ...
    public:
      ...
      void Print() {
        ...
        streamlog_out(DEBUG0) << "PlaneID" << std::setw(10)
                              << setiosflags(std::ios::right) << _PlaneID
                              << std::endl;
        streamlog_out(DEBUG0) << "X:" << std::setw(10) << std::setprecision(6)
                              << setiosflags(std::ios::right) << _x
                              << std::endl;
        streamlog_out(DEBUG0) << "Y:" << std::setw(10) << std::setprecision(6)
                              << setiosflags(std::ios::right) << _y
                              << std::endl;
        streamlog_out(DEBUG0) << "Z:" << std::setw(10) << std::setprecision(6)
                              << setiosflags(std::ios::right) << _z
                              << std::endl;
      }
    ...
    public:
      ...
      void print() {
        streamlog_out(MESSAGE4) << "Sensor: " << sensor << ": (" << A << "|"
                                << B << ") to (" << C << "|" << D << ")"
                                << std::endl;
      }
    };
    ...
    public:
      ...
      bool isInside(int s, int x, int y) {
        ...
        if (it == _rect.end()) { // not in the map means no limit on this sensor
                                 // -> always true
          return true;
        }
...
*/
    ...
    enum AlignmentMode {
      ...
      XYShiftYZRotXYRot,
      XYShiftXZRotXYRot,
      XYShiftXZRotYZRotXYRot,
      XYZShiftXZRotYZRotXYRot
    };
    ...
    struct BinomialCombination {
      ...
      DISALLOW_COPY_AND_ASSIGN(
          BinomialCombination) // prevent users from making (default) copies
      ...
      std::vector<std::vector<int>> sampleCombinations() {
        ...
        // intialise algorithm
        {
          for (int j = 1; j <= _kTaken; ++j)
            statevec.push_back(j - 1);
          statevec.push_back(_nObjects);
          statevec.push_back(0);
        }

        std::vector<std::vector<int>>
            result; // result keeps all generated combinations
