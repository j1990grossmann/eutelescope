 ...
 *
 */
#ifndef EUTELMAPSAHITMAKER_H
#define EUTELMAPSAHITMAKER_H
...
namespace eutelescope {
   ...
   *
   *
   *  @param HitCollectionName The name of the input Tracker Hit
   *  collection.
   *
   ...
   *  processing.
   *
   *  @author Author Johannes Grossmann, HEPHY<mailto:Johannes.Grossmann@oeaw.ac.at>
   *  @version $Id$
   *
   */

  class EUTelMAPSAHitMaker : public marlin::Processor {

  private:
    DISALLOW_COPY_AND_ASSIGN(EUTelMAPSAHitMaker)

  public:
    //! Returns a new instance of EUTelMAPSAHitMaker
    /*! This method returns a new instance of this processor.  It is
     *  called by Marlin execution framework and it shouldn't be
     *  called/used by the final user.
     *
     *  @return a new EUTelMAPSAHitMaker.
     */
    virtual Processor *newProcessor() { return new EUTelMAPSAHitMaker; }
    ...
    /*! This method loops over all Events in a given lcio file with TRACKERHIT collections
     *  finished.
     */
  ...
  protected:
    //! MAPSA Cluster Root file
    /*! This is the name of the root file where the MAPSA Clusters are stored.
     */
    boost::timer::auto_cpu_timer* _timer;
    std::string _RootFilename;
    std::string _ShiftFilename;
    std::string _DataPath;
    
    /*! This is the name of the collection holding the input hit
  ...
  private:
    //! Run number
    int _iRun;
    //! Negative initial shift
    int _neg_shift;
    //! Event number
    int _iEvt;
    int _total_evts_cms_ref_file;

    //! Input file with mapsa cluster data in local frame of reference
    TFile* _input_file;
    //! Input file with with goodflaghistogram
    TFile* _input_file_corr;
    //! TTreeReader for MAPSA Data
    TTreeReader* _reader;
    //! TTreeReader for shiftvectors
    TTreeReader* _reader_corr;
    //! TTreeReaderValue for counter clusters     
    TTreeReaderValue<std::vector<CounterCluster>>* _f_counter_cluster;
    TH1S* _goodflaghist; 
    

    //! Conversion ID map. Probably not needed
...
//     std::map<int, int> _conversionIdMap;
    ...
    //! Vector of shifts for MAPSA and CMSPixelRefPlane
    /*  This vector is set during the initialization procedure
     */
    std::vector<int> _shift_mpa;
    //! Vector of shifts for MAPSA and CMSPixelRefPlane
    /*  This vector is set during the initialization procedure
     */
    std::vector<int> _shift_pixel;
    //! The IO::LCReader Object
    /*  This object is used to read the the hitcollection to be merged
     */
    std::unique_ptr<IO::LCReader> _cms_ref_syncreader;   
    ...
    bool CheckValue(const ROOT::Internal::TTreeReaderValueBase& value);
    void getShift(unsigned int evtno, bool& goodflag, int& mapsa_evt_no, int& cms_ref_evt_no);
    void checkFile(const boost::filesystem::path& p);
  };

  //! A global instance of the processor
  EUTelMAPSAHitMaker gEUTelMAPSAHitMaker;
}
#endif
#endif
