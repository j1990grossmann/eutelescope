...
EUTelClusterFilter::EUTelClusterFilter() : Processor("EUTelClusterFilter") {
  ...
  _description =
      "EUTelClusterFilter is a very powerful tool. It allows to select among "
      "an input collection of TrackerPulse\n"
      "only the clusters fulfilling a certain set of selection criteria.\n"
      "The user can modify the switch on and off each selection cut and set "
      "the proper value for that via the processor parameter.";

  // first of all we need to register the input collection
  registerInputCollection(
      LCIO::TRACKERPULSE, "InputPulseCollectionName",
  ...
  registerInputCollection(LCIO::TRACKERDATA, "NoiseCollectionName",
                          ...
                          "The presence of this collection in the event is "
                          "allowing all the noise based selection cuts",
  ...
  registerOutputCollection(LCIO::TRACKERPULSE, "OutputPulseCollectionName",
                           "This is the output Tracker Pulse collection "
                           "containing the filtered clusters",
                           _outputPulseCollectionName,
                           string("filteredcluster"));
  ...
  minNChargeVecExample.push_back(
      9); // this is the number of pixels in the cluster
  ...
  registerProcessorParameter(
      ...
      "The first figure has to be the number of pixels to consider in the "
      "cluster, \n"
  ...
  registerProcessorParameter(
      ...
      "The first figure has to be the number of pixels to consider in the "
      "cluster, \n"
      "then one float number for each sensor. Setting N = 0 is enough to "
      "disable the cut.",
  ...
  registerProcessorParameter(
      "ClusterNxNMinCharge",
      "This is the minimum charge that a cluster of N times N pixels has to "
      "have.\n"
      "The first figure is the subcluster size in pixels (odd number), then "
      "one floating number for each \n"
  ...
  registerProcessorParameter(
      "ClusterNxNMinSNR", "This is the minimum SNR that a cluster of N times N "
                          "pixels has to have.\n"
                          "The first figure is the subcluster size in pixels "
                          "(odd number), then one floating number for each \n"
                          "planes. To switch this selection off, set at least "
  ...
  registerProcessorParameter("SeedMinCharge",
                             "This is the minimum allowed charge that the seed "
                             "pixel of a cluster has to have.\n"
  ...
  registerProcessorParameter(
      "SeedMinSNR",
      "This is the minimum allowed SNR that the seed pixel of a cluster has to "
      "have.\n"
  ...
  registerProcessorParameter(
      ...
      "One integer number for each detector. Write a negative number to "
      "disable the cut",
  ...
  registerProcessorParameter("MaxClusterNoise",
                             "This is maximum allowed cluster noise.\n"
                             "One floating number for each detector. Write a "
                             "negative number to disable the cut",
  ...
  registerProcessorParameter(
      "DFFNumberOfHits", "This is a cut on the number of hit pixels inside the "
                         "digital fixed frame\n"
...
void EUTelClusterFilter::init() {
  ...
  if (count_if(_minTotalChargeVec.begin(), _minTotalChargeVec.end(),
               bind2nd(greater<float>(), 0)) != 0) {
  ...
  streamlog_out(DEBUG2) << "MinTotalChargeSwitch " << _minTotalChargeSwitch
                        << endl;
  ...
  // NxN cluster charge
  if ((count_if(_minNxNChargeVec.begin(), _minNxNChargeVec.end(),
                bind2nd(greater<float>(), 0)) != 0) &&
  ...
  // NxN cluster SNR
  if ((count_if(_minNxNSNRVec.begin(), _minNxNSNRVec.end(),
                bind2nd(greater<float>(), 0)) != 0) &&
      (_minNxNSNRVec[0] != 0)) {
  ...
  // Seed charge
  if (count_if(_minSeedChargeVec.begin(), _minSeedChargeVec.end(),
               bind2nd(greater<float>(), 0)) != 0) {
  ...
  streamlog_out(DEBUG2) << "MinSeedChargeSwitch " << _minSeedChargeSwitch
                        << endl;

  // Seed SNR
  if (count_if(_minSeedSNRVec.begin(), _minSeedSNRVec.end(),
               bind2nd(greater<float>(), 0)) != 0) {
  ...
  // Cluster quality
  if (count_if(_clusterQualityVec.begin(), _clusterQualityVec.end(),
  ...
  streamlog_out(DEBUG2) << "ClusterQualitySwitch " << _clusterQualitySwitch
                        << endl;

  // Minimum cluster SNR
  if (count_if(_minTotalSNRVec.begin(), _minTotalSNRVec.end(),
               bind2nd(greater<float>(), 0)) != 0) {
  ...
  if (count_if(_minClusterNoVec.begin(), _minClusterNoVec.end(),
  ...
  if (count_if(_maxClusterNoVec.begin(), _maxClusterNoVec.end(),
               bind2nd(greater<int>(), 0)) != 0) {
  ...
  if (count_if(_DFFNHitsCuts.begin(), _DFFNHitsCuts.end(),
               bind2nd(greater<int>(), 0)) != 0) {
  ...
  // max cluster noise
  if (count_if(_maxClusterNoiseVec.begin(), _maxClusterNoiseVec.end(),
               bind2nd(greater_equal<float>(), 0)) != 0) {
    _maxClusterNoiseSwitch = true;
  } else {
    _maxClusterNoiseSwitch = false;
  }
  streamlog_out(DEBUG2) << "MaxClusterNoiseSwitch " << _maxClusterNoiseSwitch
                        << endl;

  streamlog_out(DEBUG2) << "SameNumberofHitSwitch " << _sameNumberOfHitSwitch
                        << endl;
...
void EUTelClusterFilter::initializeGeometry(LCEvent *event) {
  ...
  try {
    ...
    LCCollectionVec *noiseCollection = dynamic_cast<LCCollectionVec *>(
    ...
    for (size_t iDetector = 0; iDetector < noiseCollection->size();
         ++iDetector) {
      TrackerDataImpl *noise = dynamic_cast<TrackerDataImpl *>(
          noiseCollection->getElementAt(iDetector));
...
void EUTelClusterFilter::checkCriteria() {
  ...
  if (_minTotalChargeSwitch) {
    if (_minTotalChargeVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The threshold vector on the total cluster charge did not match "
             "the right size \n"
      ...
      streamlog_out(DEBUG1)
          << "Total cluster charge criterion verified and switched on" << endl;
  ...
  if (_minTotalSNRSwitch) {
    if (_minTotalSNRVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The threshold vector on the total cluster SNR did not match the "
             "right size \n"
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1)
          << "Total cluster SNR criterion verified and switched on" << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_minNChargeSwitch) {
    ...
    if (_minNChargeVec.size() % module != 0) {
      streamlog_out(ERROR1)
          << "The threshold vector for the N pixels charge did not match the "
             "right size \n "
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1)
          << "N pixel charge criterion verified and switched on" << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_minNSNRSwitch) {
    ...
    if (_minNSNRVec.size() % module != 0) {
      streamlog_out(ERROR1)
          << "The threshold vector for the N pixels SNR did not match the "
             "right size \n "
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1) << "N pixel SNR criterion verified and switched on"
                            << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_minNxNChargeSwitch) {
    ...
    if (_minNxNChargeVec.size() % module != 0) {
      streamlog_out(ERROR1)
          << "The threshold vector for the N x N pixels charge did not match "
             "the right size \n "
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1)
          << "NxN pixel charge criterion verified and switched on" << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_minNxNSNRSwitch) {
    ...
    if (_minNxNSNRVec.size() % module != 0) {
      streamlog_out(ERROR1)
          << "The threshold vector for the N x N pixels SNR did not match the "
             "right size \n "
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1)
          << "NxN pixel SNR criterion verified and switched on" << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_minSeedChargeSwitch) {
    if (_minSeedChargeVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The threshold vector on the seed charge did not match the right "
             "size \n"
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1) << "Seed charge criterion verified and switched on"
                            << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_minSeedSNRSwitch) {
    if (_minSeedSNRVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The threshold vector on the seed SNR did not match the right "
             "size \n"
          << "The number of planes is " << _noOfDetectors
          << " while the thresholds are " << _minSeedSNRVec.size() << "\n"
          << "Disabling the selection criterion and continue without" << endl;
      _minSeedSNRSwitch = false;
    } else {
      streamlog_out(DEBUG1) << "Seed SNR criterion verified and switched on"
                            << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_clusterQualitySwitch) {
    if (_clusterQualityVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The cluster quality vector did not match the right size \n"
          << "The number of planes is " << _noOfDetectors
          << " while the thresholds are " << _minSeedChargeVec.size() << "\n"
          << "Disabling the selection criterion and continue without" << endl;
      _clusterQualitySwitch = false;
      ...
      streamlog_out(DEBUG1)
          << "Cluster quality criterion verified and switched on" << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_minClusterNoSwitch) {
    if (_minClusterNoVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The minimum cluster number vector did not match the right size \n"
          << "The number of planes is " << _noOfDetectors
          << " while the thresholds are " << _minClusterNoVec.size() << "\n"
          << "Disabling the selection criterion and continue without" << endl;
      _minClusterNoSwitch = false;
    } else {
      streamlog_out(DEBUG1)
          << "Minimum cluster number criterion verified and switched on"
          << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_maxClusterNoSwitch) {
    if (_maxClusterNoVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The maximum cluster number vector did not match the right size \n"
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1)
          << "Maximum cluster number criterion verified and switched on"
          << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_maxClusterNoiseSwitch) {
    if (_maxClusterNoiseVec.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The maximum cluster noise vector did not match the right size \n"
          << "The number of planes is " << _noOfDetectors
      ...
      streamlog_out(DEBUG1)
          << "Maximum cluster noise criterion verified and switched on" << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_dffnhitsswitch) {
    if (_DFFNHitsCuts.size() != static_cast<unsigned>(_noOfDetectors)) {
      streamlog_out(ERROR1)
          << "The minimum hit pixel vector did not match the right size \n"
          << "The number of planes is " << _noOfDetectors
          << " while the thresholds are " << _DFFNHitsCuts.size() << "\n"
          << "Disabling the selection criterion and continue without" << endl;
      _dffnhitsswitch = false;
    } else {
      streamlog_out(DEBUG1)
          << "Minimum hit pixel number criterion verified and switched on"
          << endl;
      vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_insideROISwitch) {
    streamlog_out(DEBUG1) << "Inside ROI criterion verified and switched on"
                          << endl;
    vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_outsideROISwitch) {
    streamlog_out(DEBUG1) << "Outside ROI criterion verified and switched on"
                          << endl;
    vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
  ...
  if (_sameNumberOfHitSwitch) {
    streamlog_out(DEBUG1)
        << "Same number of hits criterion verified and switched on" << endl;
    vector<unsigned int> rejectedCounter(_noOfDetectors, 0);
...
void EUTelClusterFilter::processRunHeader(LCRunHeader *rdr) {
  ...
  if (isFirstEvent()) {
    unique_ptr<EUTelRunHeaderImpl> runHeader =
...
void EUTelClusterFilter::processEvent(LCEvent *event) {
  ...
  try {
    LCCollectionVec *pulseCollectionVec = dynamic_cast<LCCollectionVec *>(
        evt->getCollection(_inputPulseCollectionName));
    LCCollectionVec *filteredCollectionVec =
        new LCCollectionVec(LCIO::TRACKERPULSE);
    CellIDEncoder<TrackerPulseImpl> outputEncoder(
        EUTELESCOPE::PULSEDEFAULTENCODING, filteredCollectionVec);
    ...
    // CLUSTER BASED CUTS
    for (int iPulse = 0; iPulse < pulseCollectionVec->getNumberOfElements();
         iPulse++) {
      streamlog_out(DEBUG1) << "Filtering cluster " << iPulse + 1 << " / "
                            << pulseCollectionVec->getNumberOfElements()
                            << endl;
      TrackerPulseImpl *pulse = dynamic_cast<TrackerPulseImpl *>(
          pulseCollectionVec->getElementAt(iPulse));
      ClusterType type = static_cast<ClusterType>(
          static_cast<int>(inputDecoder(pulse)["type"]));
      ...
      if (type == kEUTelDFFClusterImpl) {
        ...
        if (_noiseRelatedCuts) {
          ...
          //! ---
          //! ((NOTE TAKI)): actually i think each cluster does contain its own
          //! noise values already!
          //! each candidate, that was created in clusearch, already had its
          //! noise set properly!
          //! is this information kept when storing the clusters inside the
          //! corresponding collection?
          //! not sure what happened to the status, as well!
          //! well... this routine here seems to set the noise again but will
          //! set noise = 0, if a pixel is a bad one.
          //! this might cause the bricked cluster to see some pixels with noise
          //! = 0 again!
          //! ---
          ...
          try {
            ...
            TrackerDataImpl *noiseMatrix = dynamic_cast<TrackerDataImpl *>(
                noiseCollectionVec->getElementAt(detectorPos));
            TrackerRawDataImpl *statusMatrix =
                dynamic_cast<TrackerRawDataImpl *>(
                    statusCollectionVec->getElementAt(detectorPos));
            ...
            for (int yPixel = ySeed - (yClusterSize / 2);
                 yPixel <= ySeed + (yClusterSize / 2); yPixel++) {
              for (int xPixel = xSeed - (xClusterSize / 2);
                   xPixel <= xSeed + (xClusterSize / 2); xPixel++) {

                // always check we are still within the sensor!!!
                if ((xPixel >= noiseMatrixDecoder.getMinX()) &&
                    (xPixel <= noiseMatrixDecoder.getMaxX()) &&
                    (yPixel >= noiseMatrixDecoder.getMinY()) &&
                    (yPixel <= noiseMatrixDecoder.getMaxY())) {
                  int index = noiseMatrixDecoder.getIndexFromXY(xPixel, yPixel);

                  // the corresponding position in the status matrix has to be
                  // HITPIXEL
                  ...
                  //
                  if (static_cast<int>(statusMatrix->getADCValues().size()) >
                      index) {
                    bool isBad = (statusMatrix->getADCValues()[index] ==
                                  EUTELESCOPE::BADPIXEL);
                    if (!isBad) {
                      noiseValues.push_back(
                          noiseMatrix->getChargeValues()[index]);
                    } else {
            ...
            streamlog_out(ERROR1) << e.what() << endl
                                  << "Continuing w/o noise based cuts" << endl;
        ...
        TrackerDataImpl *oneCluster = dynamic_cast<TrackerDataImpl *>(
            sparseClusterCollectionVec->getElementAt(0));
        CellIDDecoder<TrackerDataImpl> anotherDecoder(
            sparseClusterCollectionVec);
        pixelType = static_cast<SparsePixelType>(
            static_cast<int>(anotherDecoder(oneCluster)["sparsePixelType"]));
        ...
        if (pixelType == kEUTelGenericSparsePixel) {
          ...
          auto recasted =
              dynamic_cast<EUTelSparseClusterImpl<EUTelGenericSparsePixel> *>(
                  cluster);
          ...
          if (_noiseRelatedCuts) {
            ...
            try {
              ...
              for (auto &sparsePixel : pixelVec) {
                int index = noiseMatrixDecoder.getIndexFromXY(
                    sparsePixel.getXCoord(), sparsePixel.getYCoord());
          ...
          streamlog_out(ERROR4) << "Unknown pixel type. Sorry for quitting"
                                << endl;
...
//        isAccepted &= isBelowMaxClusterNoise(cluster);
      ...
      if (isAccepted)
        acceptedClusterVec.push_back(iPulse);
    ...
    while (cluIter != acceptedClusterVec.end()) {
      TrackerPulseImpl *pulse = dynamic_cast<TrackerPulseImpl *>(
          pulseCollectionVec->getElementAt(*cluIter));
    ...
    if (!isEventAccepted)
      acceptedClusterVec.clear();

    if (acceptedClusterVec.empty()) {
      delete filteredCollectionVec;
      if (!areClusterEnoughTemp) {
        streamlog_out(DEBUG1) << "Not enough clusters passed the selection "
                              << endl;
      } else if (areClusterTooManyTemp) {
        streamlog_out(DEBUG1) << "Too many clusters passed the selection "
                              << endl;
      } else {
      ...
      if (_skipEmptyEvent) {
        if (!areClusterEnoughTemp) {
          streamlog_out(WARNING0)
              << "Skipping event because too few clusters passed the selection"
              << endl;
        } else if (areClusterTooManyTemp) {
          streamlog_out(WARNING0)
              << "Skipping event because too many clusters passed the selection"
              << endl;
        } else {
          streamlog_out(WARNING0)
              << "Skipping event because no clusters passed the selection"
              << endl;
        }
      ...
      while (iter != acceptedClusterVec.end()) {
        TrackerPulseImpl *pulse = dynamic_cast<TrackerPulseImpl *>(
            pulseCollectionVec->getElementAt(*iter));
        ...
        _acceptedClusterCounter[_ancillaryIndexMap[inputDecoder(
            pulse)["sensorID"]]]++;
        ++iter;
      }
    ...
    streamlog_out(MESSAGE2)
        << "Input collection not found in the current event." << endl;
    return;
  }
}
  ...
  for (size_t iDetector = 0; iDetector < _noOfDetectors; iDetector++) {
    ...
    if (clusterNoVec[iDetector] < _minClusterNoVec[iDetector]) {
      streamlog_out(DEBUG2)
          << "Rejected event because on detector " << iDetector << " there are "
          << clusterNoVec[iDetector] << "\nwhile "
...
bool EUTelClusterFilter::areClusterTooMany(
    std::vector<int> clusterNoVec) const {
...
bool EUTelClusterFilter::hasSameNumberOfHit(
    std::vector<int> clusterNoVec) const {
  ...
  while (iter != clusterNoVec.end()) {
    if (*(iter - 1) != (*iter)) {
      streamlog_out(DEBUG2) << "Rejected event because the number of hit is "
                               "different from one sensor to the other"
                            << endl;
      hasSameNumber = false;
      break;
    }
  ...
  if (!hasSameNumber) {
    ...
    vector<unsigned int>::iterator iter2 =
        _rejectionMap["SameNumberOfHitCut"].begin();
    while ((iter != clusterNoVec.end()) ||
...
bool EUTelClusterFilter::isAboveNumberOfHitPixel(
    EUTelVirtualCluster *cluster) const {
  if (!_dffnhitsswitch) {
    return true;
  }
  streamlog_out(DEBUG1)
      << "Filtering against number of hit pixel inside a cluster " << endl;

  int detectorID = cluster->getDetectorID();
  int detectorPos = _ancillaryIndexMap[detectorID];

  if (static_cast<int>(cluster->getTotalCharge()) >= _DFFNHitsCuts[detectorPos])
    return true;
  else {
    streamlog_out(DEBUG2)
        << "Rejected cluster because the number of hit pixel is "
        << static_cast<int>(cluster->getTotalCharge())
...
bool EUTelClusterFilter::isAboveMinTotalCharge(
    EUTelVirtualCluster *cluster) const {
  ...
  if (cluster->getTotalCharge() > _minTotalChargeVec[detectorPos])
    return true;
  else {
    streamlog_out(DEBUG2) << "Rejected cluster because its charge is "
                          << cluster->getTotalCharge()
                          << " and the threshold is "
                          << _minTotalChargeVec[detectorPos] << endl;
    _rejectionMap["MinTotalChargeCut"][detectorPos]++;
    return false;
  }
}

bool EUTelClusterFilter::isAboveMinTotalSNR(
    EUTelVirtualCluster *cluster) const {
  ...
  if (cluster->getClusterSNR() > _minTotalSNRVec[detectorPos])
    return true;
  else {
    streamlog_out(DEBUG2) << "Rejected cluster because its SNR is "
                          << cluster->getClusterSNR()
                          << " and the threshold is "
                          << _minTotalSNRVec[detectorPos] << endl;
...
bool EUTelClusterFilter::isAboveNMinCharge(EUTelVirtualCluster *cluster) const {
  ...
  while (iter != _minNChargeVec.end()) {
    ...
    if (charge > threshold) {
      ...
      streamlog_out(DEBUG2) << "Rejected cluster because its charge over "
                            << (*iter) << " is " << charge
...
bool EUTelClusterFilter::isAboveNMinSNR(EUTelVirtualCluster *cluster) const {
  ...
  while (iter != _minNSNRVec.end()) {
    ...
    if (SNR > threshold) {
      ...
      streamlog_out(DEBUG2) << "Rejected cluster because its SNR over "
                            << (*iter) << " is " << SNR
                            << " and the threshold is " << threshold << endl;
...
bool EUTelClusterFilter::isAboveNxNMinCharge(
    EUTelVirtualCluster *cluster) const {
  ...
  while (iter != _minNxNChargeVec.end()) {
    ...
    if ((threshold <= 0) || (charge > threshold)) {
      ...
      streamlog_out(DEBUG2) << "Rejected cluster because its charge within a "
                            << (*iter) << " x " << (*iter) << " subcluster is "
                            << charge << " and the threshold is " << threshold
                            << endl;
...
bool EUTelClusterFilter::isAboveNxNMinSNR(EUTelVirtualCluster *cluster) const {
  ...
  while (iter != _minNxNSNRVec.end()) {
    ...
    if ((threshold <= 0) || (snr > threshold)) {
      ...
      streamlog_out(DEBUG2) << "Rejected cluster because its SNR within a "
                            << (*iter) << " x " << (*iter) << " subcluster is "
                            << snr << " and the threshold is " << threshold
                            << endl;
...
bool EUTelClusterFilter::isAboveMinSeedCharge(
    EUTelVirtualCluster *cluster) const {
  ...
  int detectorID = cluster->getDetectorID();
  int detectorPos = _ancillaryIndexMap[detectorID];
  if (cluster->getSeedCharge() > _minSeedChargeVec[detectorPos])
    return true;
  else {
    streamlog_out(DEBUG2) << "Rejected cluster because its seed charge is "
                          << cluster->getSeedCharge()
                          << " and the threshold is "
                          << _minSeedChargeVec[detectorPos] << endl;
...
bool EUTelClusterFilter::isAboveMinSeedSNR(EUTelVirtualCluster *cluster) const {
  ...
  int detectorID = cluster->getDetectorID();
  int detectorPos = _ancillaryIndexMap[detectorID];
  if (cluster->getSeedSNR() > _minSeedSNRVec[detectorPos])
    return true;
  else {
    streamlog_out(DEBUG2) << "Rejected cluster because its seed charge is "
                          << cluster->getSeedSNR() << " and the threshold is "
                          << _minSeedSNRVec[detectorPos] << endl;
...
bool EUTelClusterFilter::hasQuality(EUTelVirtualCluster *cluster) const {
  ...
  int detectorID = cluster->getDetectorID();
  int detectorPos = _ancillaryIndexMap[detectorID];
  if (_clusterQualityVec[detectorID] < 0)
    return true;

  ClusterQuality actual = cluster->getClusterQuality();
  ClusterQuality needed =
      static_cast<ClusterQuality>(_clusterQualityVec[detectorPos]);

  if (actual == needed)
    return true;
  else {
    streamlog_out(DEBUG2) << "Rejected cluster because its quality "
                          << static_cast<int>(actual) << " is not "
                          << _clusterQualityVec[detectorPos] << endl;
    _rejectionMap["ClusterQualityCut"][detectorPos]++;
    return false;
  }
}

bool EUTelClusterFilter::isBelowMaxClusterNoise(
    EUTelVirtualCluster *cluster) const {
  ...
  streamlog_out(DEBUG1) << "Filtering against the maximum cluster noise"
                        << endl;
  int detectorID = cluster->getDetectorID();
  int detectorPos = _ancillaryIndexMap[detectorID];
  if ((cluster->getClusterNoise() < _maxClusterNoiseVec[detectorPos]) ||
      (_maxClusterNoiseVec[detectorID] < 0))
    return true;
  else {
    streamlog_out(DEBUG2) << "Rejected cluster because its noise is "
                          << cluster->getClusterNoise()
                          << " and the threshold is "
                          << _maxClusterNoiseVec[detectorPos] << endl;
...
string EUTelClusterFilter::printSummary() const {
  ...
  while (iter != _rejectionMap.end()) {
    ss << " " << setiosflags(ios::left) << setw(bigSpacer) << (*iter).first
  ...
  ss << singleLine.str() << "\n"
     << " " << setiosflags(ios::left) << setw(bigSpacer) << "Accepted clusters "
     << resetiosflags(ios::left);
  ...
  ss << "\n"
     << singleLine.str() << "\n"
     << " " << setw(bigSpacer) << "Total no of clusters"
     << resetiosflags(ios::left);
